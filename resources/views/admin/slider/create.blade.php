    ...
    <div class="bg-surface-container-lowest rounded-xl shadow-sm p-5 mb-6">
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="3" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">{{ old('description') }}</textarea>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="button_target" value="_same" {{ old('button_target', '_same') === '_same' ? 'checked' : '' }} class="w-4 h-4 text-primary">
                        <span class="text-sm">{{ __t('admin.slider.button_target_same') }}</span>
                    </label>
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="button_target" value="_blank" {{ old('button_target') === '_blank' ? 'checked' : '' }} class="w-4 h-4 text-primary">
            ...
            <div class="md:col-span-2">
                ...
                <select name="animation_effect" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
                    @foreach(\App\Models\Content\Slide::ANIMATION_EFFECTS as $value => $label)
                        <option value="{{ $value }}" {{ old('animation_effect') === $value ? 'selected' : '' }}>{{ __t('admin.slider.animation_' . $value) }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <select name="entrance_effect" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
                    @foreach(\App\Models\Content\Slide::ENTRANCE_EFFECTS as $value => $label)
                        <option value="{{ $value }}" {{ old('entrance_effect') === $value ? 'selected' : '' }}>{{ __t('admin.slider.entrance_' . $value) }}</option>
                    @endforeach
                </select>
            ...
            <div>
                <label class="block text-xs font-semibold mb-1 text-on-surface-variant">{{ __t('admin.customize.or_url') }}</label>
                <input type="url" name="mobile_image" value="{{ old('mobile_image') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary font-mono text-sm" placeholder="https://..." dir="ltr">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="datetime-local" name="starts_at" value="{{ old('starts_at') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
            ...
            <div>
                ...
                <input type="datetime-local" name="ends_at" value="{{ old('ends_at') }}" class="w-full px-3 py-2 border border-outline-variant rounded-lg focus:ring-2 focus:ring-primary">
