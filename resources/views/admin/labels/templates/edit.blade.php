...
@section('title', 'تعديل قالب ملصق: ' . $template->name)
@section('page_title', 'تعديل قالب ملصق: ' . $template->name)

@section('content')
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-xl font-bold text-on-surface">تعديل قالب الملصق</h3>
    ...
    <form action="{{ route('admin.order-labels.templates.update', $template) }}" method="POST" class="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant/30 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div id="custom_dimensions_row" class="grid grid-cols-2 gap-4 {{ $template->paper_size === 'custom' ? '' : 'hidden' }}">
            <div>
                <label class="block text-xs font-bold text-on-surface mb-1">العرض (مم)</label>
                <input type="number" name="custom_width" value="{{ $template->custom_width }}" placeholder="100" class="w-full p-2.5 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary">
            </div>
            <div>
                <label class="block text-xs font-bold text-on-surface mb-1">الارتفاع (مم)</label>
                <input type="number" name="custom_height" value="{{ $template->custom_height }}" placeholder="150" class="w-full p-2.5 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary">
            </div>
        </div>
        ...
        <div class="border-t border-outline-variant pt-4 space-y-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_barcode]" value="1" {{ $template->getSetting('show_barcode') ? 'checked' : '' }} class="rounded border-outline-variant text-primary">
                    إظهار الـ Barcode
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_qr]" value="1" {{ $template->getSetting('show_qr') ? 'checked' : '' }} class="rounded border-outline-variant text-primary">
                    إظهار الـ QR Code
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_total]" value="1" {{ $template->getSetting('show_total') ? 'checked' : '' }} class="rounded border-outline-variant text-primary">
                ...
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_product_count]" value="1" {{ $template->getSetting('show_product_count') ? 'checked' : '' }} class="rounded border-outline-variant text-primary">
                    إظهار عدد المنتجات
                </label>
            </div>
        </div>
