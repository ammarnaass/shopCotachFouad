...
<div class="mb-8">
    <div class="flex items-center gap-2.5">
        ...
        <div>
            <h1 class="text-2xl sm:text-3xl font-extrabold text-on-surface tracking-tight">{{ __t('admin.coupons.create_new') }}</h1>
            <p class="text-on-surface-variant text-xs sm:text-sm mt-0.5">{{ __t('admin.coupons.manage') }}</p>
        </div>
    </div>
</div>

<form method="POST" action="{{ route('admin.coupons.store') }}" class="max-w-4xl">
    @csrf
    <div class="bg-surface-container-lowest border border-outline-variant/40 rounded-2xl shadow-xs overflow-hidden p-6 sm:p-8">
        <div class="flex items-center gap-2 pb-4 mb-6 border-b border-outline-variant/30">
            ...
            <h2 class="font-bold text-lg text-on-surface">{{ __t('admin.coupons.coupon_info') }}</h2>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="relative">
                    <input type="text" name="code" value="{{ old('code') }}" required style="text-transform: uppercase" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-mono font-bold tracking-wider text-base focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('code') border-error @enderror" placeholder="SUMMER2026">
                </div>
            ...
            <div>
                ...
                <select name="type" required class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-semibold text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('type') border-error @enderror">
                    <option value="fixed" {{ old('type', 'fixed') === 'fixed' ? 'selected' : '' }}>{{ __t('admin.coupons.fixed_amount') }} (د.ج)</option>
            ...
            <div>
                ...
                <input type="number" name="value" value="{{ old('value') }}" required min="0" step="0.01" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-bold text-base focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('value') border-error @enderror" placeholder="500">
            ...
            <div>
                ...
                <input type="number" name="min_order" value="{{ old('min_order') }}" min="0" step="0.01" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-medium text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('min_order') border-error @enderror" placeholder="2000">
            ...
            <div>
                ...
                <input type="number" name="max_discount" value="{{ old('max_discount') }}" min="0" step="0.01" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-medium text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('max_discount') border-error @enderror" placeholder="اختياري لنسبة الخصم">
            ...
            <div>
                ...
                <input type="number" name="usage_limit" value="{{ old('usage_limit') }}" min="1" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-medium text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('usage_limit') border-error @enderror" placeholder="اتركه فارغاً للاستخدام غير المحدود">
            ...
            <div>
                ...
                <input type="date" name="expiry_date" value="{{ old('expiry_date') }}" class="w-full px-4 py-3 rounded-xl border border-outline-variant/60 bg-surface-container-lowest text-on-surface font-medium text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all @error('expiry_date') border-error @enderror">
        ...
        <div class="mt-8 pt-6 border-t border-outline-variant/30 flex items-center gap-3">
            <button type="submit" class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-primary text-on-primary font-bold text-sm shadow-sm hover:shadow-md hover:brightness-105 active:scale-[0.98] transition-all">
                ...
                <span>{{ __t('admin.coupons.save') }}</span>
            </button>
