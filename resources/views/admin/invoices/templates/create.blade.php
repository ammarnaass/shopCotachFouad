...
@section('title', 'إنشاء قالب فاتورة جديد')
@section('page_title', 'إنشاء قالب فاتورة جديد')

@section('content')
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h3 class="text-xl font-bold text-on-surface">إعدادات قالب الفاتورة الجديد</h3>
    ...
    <form action="{{ route('admin.invoices.templates.store') }}" method="POST" class="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant/30 space-y-6">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-xs font-bold text-on-surface mb-1">اسم القالب</label>
                <input type="text" name="name" required placeholder="مثال: القالب الكلاسيكي المطور" class="w-full p-2.5 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary">
            </div>
            <div>
        ...
        <div>
            ...
            <input type="text" name="description" placeholder="وصف مختصر لمجال استخدام هذا القالب" class="w-full p-2.5 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary">
        </div>
        ...
        <div class="border-t border-outline-variant pt-4 space-y-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_sku]" value="1" checked class="rounded border-outline-variant text-primary">
                    إظهار SKU المنتجات
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_product_image]" value="1" class="rounded border-outline-variant text-primary">
                    إظهار صور المنتجات
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_discount]" value="1" checked class="rounded border-outline-variant text-primary">
                    إظهار الخصم
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
                    <input type="checkbox" name="settings[show_shipping]" value="1" checked class="rounded border-outline-variant text-primary">
                    إظهار سعر الشحن
                </label>
                <label class="flex items-center gap-2 text-xs font-semibold text-on-surface cursor-pointer">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-outline-variant pt-4">
            <div>
                ...
                <input type="color" name="settings[primary_color]" value="#004ac6" class="w-full h-10 p-1 border border-outline-variant rounded-xl">
            </div>
            <div>
                <label class="block text-xs font-bold text-on-surface mb-1">رسالة الشكر بالفوتر</label>
                <input type="text" name="settings[thank_you_message]" value="شكراً لتسوقكم من متجرنا" class="w-full p-2.5 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary">
            </div>
        </div>
