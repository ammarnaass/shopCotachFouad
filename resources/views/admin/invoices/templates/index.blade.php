...
@section('page_title', 'إدارة قوالب الفواتير')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-xl font-bold text-on-surface">قوالب الفواتير</h3>
            <p class="text-sm text-on-surface-variant">اختر القالب الافتراضي أو أنشئ قوالب مخصصة للطباعة و PDF</p>
        </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-outline-variant/30 relative flex flex-col justify-between">
                <div>
                    ...
                    <div class="space-y-2 text-xs text-on-surface-variant my-4 bg-surface-container-low p-3 rounded-xl">
                        <div class="flex justify-between"><span>مقاس الورق:</span><strong class="text-on-surface">{{ $template->paper_size_label }}</strong></div>
                        <div class="flex justify-between"><span>الحالة:</span><strong class="{{ $template->status ? 'text-emerald-600' : 'text-error' }}">{{ $template->status ? 'نشط' : 'معطل' }}</strong></div>
                    </div>
                </div>

                <div class="flex items-center justify-between pt-4 border-t border-outline-variant">
                    <div class="flex items-center gap-2">
                        <a href="{{ route('admin.invoices.templates.preview', [$template, 'print' => 1]) }}" target="_blank"
                    ...
                    @if(!$template->is_default)
                        <form action="{{ route('admin.invoices.templates.default', $template) }}" method="POST">
                            @csrf
